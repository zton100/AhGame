...
- Done: Issue 015 `level_curves.json`, `LevelCurve`, `LevelService`, multi-level experience gains, max level caps, and level-based class stat growth.
- Done: Issue 016 first slice: `StatAggregationService`, `ComputedStats`, stat breakdowns, and flat/percent/more/less stacking.
- Done: Issue 017 first slice: `formula_config.json`, `FormulaConfig`, `DamageFormulaService`, critical cap, resistance mitigation, armor mitigation, and damage breakdowns.
- Done: Issue 018 first slice: `EquipmentSlot`, `EquipmentTemplate`, `QualityConfig`, `EquipmentTemplateService`, and `QualityService`.
- Done: Issue 019 first slice: `EquipmentInstance`, deterministic `EquipmentGenerationService`, stable instance ids, rolled base stats, and JSON round trip.
- Done: Issue 020 first slice: `EquipmentLoadout` and `EquipmentService` equip/unequip with class and level validation.
- Done: Issue 021 first slice: `AffixConfig`, `AffixRollService`, deterministic weighted rolls, level/tag filtering, optional exclusive groups, stepped roll values, and seed data parsing.
- Done: Issue 022 first slice: `AffixEffectResolver` parses stat modifiers, status effects, event triggers, missing roll warnings, and unknown effect warnings.
- Done: Issue 023 first slice: `BuildConfig` and `BuildService` aggregate class, skill, equipment, and affix tags, then identify poison-shadow, summon-undead, or mixed builds.
- Done: Issue 024 first slice: `BuildScoreService` and `EquipmentCompareService` score equipment against current BD tags, separate match score from attack score, penalize rejected tags, and compare candidate vs equipped items.
- Done: Issue 058 first slice: `EquipmentCardViewModelFactory` exposes quality color, base stats, affixes, mechanic highlight flags, BD match score, replacement deltas, and recommendation labels without putting business logic in widgets.
- Done: Issue 033 first slice: `InventoryState`, `MaterialStack`, and `InventoryService` support equipment capacity, material stacking, JSON round trip, and legacy `InventorySave` compatibility.
- Done: Issue 033 second slice: `LootDrop` and `LootInventoryService` apply equipment/material loot to inventory, reject full equipment inventory safely, and ignore unsupported drop types for later systems.
- Done: Issue 031/033 drop slice: `DropPoolService` rolls deterministic weighted loot drops from `drop_pools.json`, including quantity ranges and unsupported-type passthrough.
- Done: Issue 033 third slice: `EquipmentLootMaterializationService` turns equipment template loot into generated `EquipmentInstance` values and inventory-ready equipment instance drops.
- Remaining: settings-page manual save/export/import placeholders, deeper save validation for future gameplay fields, S04 UI character page integration, later S06 status/difficulty formula slices, full equipment instance persistence, and equipment card UI/detail modal.
